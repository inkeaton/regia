...
**Phase 3 — Emit Playbook Files**: For each Playbook, `_emit_playbook_file()` generates a standalone `.asl` containing static-gated plans for each `PbWhenBlock`. The gate condition is `playbook_active(playbook_name, _)`, which must be true for any plan to fire.

**Phase 4 — Emit Director Files**: `_emit_director()` generates the Director agent `.asl` for each Plot. This is the most complex output file, containing:
