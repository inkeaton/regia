// ============================================
// Director agent for Plot: HonorDuel
// ============================================
...
@boot__HonorDuel[atomic]
+!boot <-
    .my_name(Me);
    +plot_id(Me);
    .print("[HonorDuel] Booting up director agent ", Me, " in phase 'combat'...");
    !on_enter(combat).

// == Director WHEN Plans ==
@dir__HonorDuel__duel_resolved__0
+duel_resolved : current_phase(combat) <-
    .print("The duel is over.");
    !end_plot.
...
+!switch_phase(Target) : current_phase(Current) <-
    .print("[HonorDuel] Transitioning from phase '", Current, "' to '", Target, "'...");
    !on_exit(Current);
...
+!parent_ended[source(Parent)] : parent_plot(Parent) <-
    .print("[HonorDuel] Parent plot ", Parent, " ended. Terminating this subplot...");
    -parent_plot(Parent);
    !end_plot.
...
+!end_plot : current_phase(Current) <-
    .my_name(Me);
    .print("[HonorDuel] Ending plot. Running exit hooks and broadcasting termination...");
    !on_exit(Current);
...
+!start_subplot(SubplotStr, SubplotAtom, MappingsData) <-
    ...
    .print("[HonorDuel] Spawning subplot '", SubplotStr, "' as agent ", ChildId, " with bindings ", Bindings);
...
@dir__HonorDuel__send_to_role__0
...
@dir__HonorDuel__agent_died__0[atomic]
+agent_died(DeadAgent) : role_agent(Role, DeadAgent) <-
    .print("[HonorDuel] Agent ", DeadAgent, " playing role '", Role, "' has died! Cleaning registry...");
